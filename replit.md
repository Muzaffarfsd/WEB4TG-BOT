...
- **Interactive Tools**: Includes a cost calculator, portfolio examples, competitor comparison tool, and an extensive FAQ system.
...
## File Structure (17,681 lines total)
- `bot.py` (347) — Main entry point, application setup
- `src/knowledge_base.py` — SYSTEM_PROMPT with 14 advanced sales sections, WELCOME_MESSAGE
- `src/context_builder.py` — 25 context signals including 12 new detectors (BANT, LPR, negotiation, velocity, sentiment, questions, budget, upsell, diversity, competitors, win-back, fatigue)
- `src/ai_client.py` — AIClient with multi-model routing, agentic loop, response validation, hallucination guard, quality checks, graceful degradation, 17 tool declarations
- `src/handlers/messages.py` (822) — Message handler with adaptive length, smart thinking routing, velocity tracking, response validation
- `src/handlers/callbacks.py` (1248) — Callback query handlers
- `src/handlers/commands.py` (526) — Command handlers
- `src/handlers/media.py` (1081) — Media/voice handlers
- `src/handlers/admin.py` (790) — Admin panel
- `src/tool_handlers.py` (437) — Tool execution including competitor comparison
- `src/advanced_analytics.py` — Analytics with drop-off analysis, conversion attribution, LTV/churn prediction
- `src/ab_testing.py` — A/B testing with significance monitoring
- `src/referrals.py` (404) — Referral system with quality scoring
- `src/propensity.py` (291) — Propensity scoring
- `src/feedback_loop.py` — Self-learning feedback loop
- `src/followup.py` — Smart follow-up with win-back
- `src/session.py` (349) — Session management
- `src/rag.py` (464) — RAG knowledge base
- `src/tasks_tracker.py` (524) — Gamification tasks
...
- **40 World-Class Improvements** implemented across all modules:
  - Enhanced SYSTEM_PROMPT with 14 new sales intelligence sections
  - 12 new context detectors in context_builder.py
  - Smart multi-model routing in ai_client.py
  - Response validation / hallucination guard
  - Competitor comparison tool (6 competitor types)
  - Advanced analytics: drop-off, LTV, churn prediction
  - A/B significance monitoring
  - Referral quality scoring
  - Adaptive response length + smart thinking routing in messages.py
