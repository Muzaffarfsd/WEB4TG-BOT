...
- **AI Integration**: Powered by Gemini 3 Flash Preview (fast) + Gemini 3 Pro Preview (thinking) with smart multi-model routing, dynamic system prompt composition (`src/prompt_composer.py`), 3 temperature tiers (0.3/0.45/0.65), predictive model selection (propensity + funnel → auto-upgrade to Pro for hot clients), cascading degradation (Pro → Flash fallback), real-time streaming, 17 function calling tools (RAG search, client memory, competitor comparison, `request_screenshot`), response validation, active response quality control (fluff removal, CTA injection, 200-word cap), and graceful degradation.
...
- **Conversation Intelligence**: Features adaptive response length (dynamic 50/80/120/180 word limits by query complexity), smart thinking level routing, response diversity, multi-intent handling, dialog repair, decision fatigue prevention, conversation velocity tracking, and sentiment analysis.
...
- **Communication & Marketing**: Features a 7-step adaptive follow-up system with AI-generated personalized messages, A/B testing per step (direct vs soft style), propensity-adaptive scheduling (3 factors: propensity score, response velocity, CTA engagement), exponential backoff for fatigue (1.5^n after 3 unanswered, cap 4x), response rate tracking per step with analytics, self-learning from successful follow-ups (auto-saved as few-shot examples), smart re-engagement triggers (CTA click → accelerate/restart sequence), CTA buttons, voice follow-ups, timezone-aware delivery, a broadcast system, and multi-language support (RU/EN/UZ/KZ auto-detection).
...
- **Voice System**: Smart adaptive voice strategy with 5-tier priority scoring (closing=100, decision=95, objection=85, price=80, sentiment=60-90). Two modes: "full" voice for short responses (<500 chars) and "bridge" voice (Progressive Disclosure — short voice hook + full text) for long responses. Sentiment-aware voice profiles (frustrated→empathy, excited→excited, rushed→bridge+factual). 5-min cooldown, max 5 per session. ElevenLabs v3 TTS with Gemini Flash transcription, emotion detection, and voice supplements for followups/broadcasts.
...
- **Self-Learning Feedback Loop v2.1**: Active learning with session attribution and outcome weighting. Auto-tags AI responses with closing technique, business niche, and communication style. AI quality evaluation via Gemini (every 5th response scored on 5 criteria). RAG on successful dialogs (`src/dialog_rag.py`) — converting conversations saved as dynamic few-shot examples.
