        ...
        try:
            ...
            if is_returning:
                ...
                greet_prompt = (
                    ...
                    "НЕ 'мы делаем мини-аппы', А 'магазину одежды запустили каталог в Телеграме — конверсия вдвое за неделю'. "
                    ...
                    "- 'Магазину одежды запустили каталог в Телеграме — конверсия выросла вдвое за неделю'\n"
                    "- 'Обувному бренду сделали витрину прямо в Телеграме — заказы полетели с первого дня'\n"
                    "- 'Ресторану подключили заказы через Телеграм — плюс 40 процентов за неделю'\n"
                    "- 'Автосервису запустили бот — клиенты сами записываются, менеджер даже не звонит'\n\n"
                    "СТРОГИЕ ПРАВИЛА:\n"
                    "- 150-250 символов (10-18 секунд). Коротко как голосовое другу, НЕ презентация\n"
        ...
        if not voice_greeting:
            if is_returning:
                ...
                voice_greeting = (
                    ...
                    f"[excited] Тут магазину одежды запустили каталог прямо в Телеграме — "
                    f"конверсия выросла вдвое за неделю. "
                    f"[whispers] Кстати, можем и голосовыми общаться. "
                    f"[curious] А вы чем занимаетесь?"
                )
