        ...
        try:
            ...
            if is_returning:
                ...
                greet_prompt = (
                    ...
                    f"ВРЕМЯ СУТОК: {time_word.lower()}. Настроение: {mood_instruction}.\n\n"
                    "ЗАДАЧА: Человек ВПЕРВЫЕ зашёл к нам в бот. Он ещё НЕ знает кто мы и зачем ему это. "
                    "У тебя 20 секунд чтобы его ЗАЦЕПИТЬ. Голосовое должно вызвать 'вау-эффект' — "
                    "'мне записали ЛИЧНОЕ голосовое, это не шаблон, это живой человек!'. "
                    "После этого голосового клиент должен ЗАХОТЕТЬ ответить и узнать больше.\n\n"
                    "ПСИХОЛОГИЯ ПЕРВОГО КОНТАКТА:\n"
                    "- Эффект неожиданности — голосовое от бота? Это сразу выделяет из толпы\n"
                    "- Экспертность через кейс — не 'мы делаем приложения', а конкретный результат: 'запустили ресторану — 40 процентов к заказам за неделю'\n"
                    "- Интрига — не рассказывай ВСЁ, оставь крючок: 'если интересно — могу показать как это работает'\n"
                    "- Вопрос о НЁМ — людям нравится говорить о себе: 'а вы чем занимаетесь?' переключает фокус на клиента\n"
                    "- НЕ продавай — заинтересовывай. Продажа отталкивает, интерес притягивает\n\n"
                    "СТРУКТУРА (20-25 секунд, золотой стандарт):\n"
                    "1. [friendly] Приветствие + имя. Легко, с улыбкой: 'Привет!' или время суток\n"
                    "2. [confident] Кто ты — ОДНА фраза: 'Я Алекс, из WEB4TG Studio'\n"
                    "3. [excited] Крючок-кейс — КОНКРЕТНЫЙ результат в ОДНОМ предложении. "
                    "НЕ 'мы делаем мини-аппы', А 'вчера салону красоты запустили запись через Телеграм — у них за день 30 бронирований'. "
                    "Цифры, ниша, результат — это цепляет\n"
                    "4. [whispers] или [playfully] Интрига — 'кстати, можем и вам такое показать' или 'у нас есть кое-что интересное'\n"
                    "5. [curious] Вопрос о клиенте — открытый, про бизнес: 'А расскажите — чем занимаетесь?'\n\n"
                    "ПРИМЕРЫ КЕЙСОВ-КРЮЧКОВ (выбери или придумай похожий):\n"
                    "- 'Вот буквально на днях ресторану запустили заказы через Телеграм — за первую неделю плюс 40 процентов'\n"
                    "- 'Салону красоты сделали онлайн-запись в Телеграме — 30 бронирований в первый же день'\n"
                    "- 'Интернет-магазину подключили каталог прямо в Телеграм — конверсия выросла вдвое'\n"
                    "- 'Автосервису запустили бот — клиенты сами записываются, менеджер даже не звонит'\n\n"
                    "СТРОГИЕ ПРАВИЛА:\n"
                    "- 200-350 символов (15-25 секунд). Больше — уже не слушают\n"
                    "- Говори как ЖИВОЙ ЧЕЛОВЕК — паузы '...', тире ' — ', одно междометие\n"
                    "- WEB4TG Studio — по-английски, как бренд\n"
                    "- ОБЯЗАТЕЛЬНО упомяни что можно и голосовыми отвечать — мимоходом\n"
                    "- НЕТ: 'чем могу помочь', 'к вашим услугам', 'обращайтесь', 'предлагаю рассмотреть'\n"
                    "- НЕТ: списки услуг, перечисления, markdown, emoji, кавычки\n"
                    "- КАЖДЫЙ раз другой кейс и другая подача — не повторяйся\n"
                    "- Верни ТОЛЬКО текст для озвучки"
                )
        ...
        if not voice_greeting:
            if is_returning:
                voice_greeting = (
                    ...
                    f"[curious] Расскажете как у вас дела? Интересно послушать."
                )
            else:
                voice_greeting = (
                    f"[friendly] {time_word}{name_part}! Я Алекс, из WEB4TG Studio. "
                    f"[confident] Мы делаем приложения прямо в Телеграме — "
                    f"[excited] вот на днях салону красоты запустили онлайн-запись... "
                    f"за первый день тридцать бронирований. Без сайта, без приложения — всё в Телеграме. "
                    f"[whispers] Кстати, можем и голосовыми общаться — мне так даже удобнее. "
                    f"[curious] А расскажите — вы чем занимаетесь?"
                )
