...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async def _send_voice_greeting_background():
        ...
        try:
            ...
            if is_returning:
                greet_prompt = (
                    ...
                    "ПРИЁМЫ ЖИВОЙ РЕЧИ:\n"
                    "- [warm] перед первой фразой — задаёт тёплый тон\n"
                    ...
                    "- Чередуй короткие и длинные фразы: 'Привет! Ну как вы? Давно не заходили — я уж думал, всё решили без меня'\n\n"
                    "СТРОГИЕ ПРАВИЛА:\n"
                    "- 150-250 символов (10-18 секунд). Приветствие КОРОТКОЕ, не лекция\n"
                    "- НЕТ markdown, emoji, списков, кавычек\n"
                    "- Теги [warm] [friendly] [excited] МОЖНО и НУЖНО использовать для эмоций\n"
                    "- НЕТ фраз: 'чем могу помочь', 'к вашим услугам', 'обращайтесь' — это робот\n"
                    "- Аббревиатуры: WEB4TG = вэб-фор-тэ-гэ\n"
                ...
                greet_prompt = (
                    ...
                    "2. Представься коротко и живо — 'Алекс, из вэб-фор-тэ-гэ' (не 'являюсь консультантом компании')\n"
                    ...
                    "ПРИЁМЫ ЖИВОЙ РЕЧИ:\n"
                    "- [friendly] перед первой фразой — дружелюбный старт\n"
                    "- Начни непринуждённо: 'Ну смотрите...', 'Короче, дело вот в чём —', 'Вот что скажу —'\n"
                    "- Паузы '...' для раздумья: 'Хм... как бы это коротко...'\n"
                    "- Тире ' — ' для акцентов: 'Прямо в Телеграме — без скачиваний'\n"
                    "- Одно 'кстати' или 'знаете что' для естественного перехода\n"
                    "- Упомяни что можно и голосовыми общаться — мимоходом, не акцентируя\n\n"
                    "СТРОГИЕ ПРАВИЛА:\n"
                    "- 200-350 символов (15-25 секунд). Первое впечатление, не презентация\n"
                    "- НЕТ markdown, emoji, списков, кавычек\n"
                    "- Теги [warm] [friendly] [excited] [confident] МОЖНО и НУЖНО использовать для эмоций\n"
                    ...
                    "- Аббревиатуры: WEB4TG = вэб-фор-тэ-гэ, Mini Apps = мини-аппс\n"
                    "- Верни ТОЛЬКО текст для озвучки"
                )
        ...
        if not voice_greeting:
            if is_returning:
                voice_greeting = (
                    f"[warm] {time_word}{name_part}! О, привет... "
                    f"Рад что заглянули. Ну как у вас — продвинулись с проектом? "
                    f"Расскажите, я тут — поможем разобраться."
                )
            else:
                voice_greeting = (
                    f"[friendly] {time_word}{name_part}! Алекс, из вэб-фор-тэ-гэ Студио. "
                    f"Ну смотрите... мы делаем приложения прямо внутри Телеграма — "
                    f"магазины, рестораны, салоны — вот буквально вчера одному ресторану запустили, заказы уже пошли. "
                    f"Кстати, можем и голосовыми общаться — как удобнее. "
                    f"А вы чем занимаетесь? Расскажете?"
                )
